...
pub struct HiveConfig {
    ...
    /// When omitted, defaults to true via [`HiveConfig::manage_gitignore`].
    #[serde(
        default,
...
pub type Workbench = Hive;
pub type WorkbenchConfig = HiveConfig;
...
pub fn load_hive(root: &Path) -> Result<Hive, HiveError> {
...
pub fn parse_config_yaml(text: &str) -> Result<HiveConfig, HiveError> {
    let trimmed = text.trim();
    if trimmed.is_empty() {
        return Ok(HiveConfig::default());
    }
    serde_yaml::from_str(trimmed)
        .map_err(|e| HiveError::hive(format!("invalid config YAML: {e}")))
}
...
fn validate_entity_names_and_paths(config: &HiveConfig) -> Result<(), HiveError> {
...
fn require_non_empty_name(kind: &str, name: &str) -> Result<(), HiveError> {
...
pub fn require_entity_name(kind: &str, name: &str) -> Result<(), HiveError> {
    match parse_path_token(name) {
        Ok(_) => Ok(()),
        Err("empty") => Err(HiveError::hive(format!(
            "{kind} name must not be empty"
        ))),
        Err("dot") => Err(HiveError::hive(format!(
            "invalid {kind} name '{}'",
            name.trim()
        ))),
        Err("separator") => Err(HiveError::hive(format!(
            "invalid {kind} name '{}': path separators not allowed",
            name.trim()
        ))),
        Err(_) => Err(HiveError::hive(format!(
...
fn validate_rel_path(kind: &str, name: &str, path: &str) -> Result<(), HiveError> {
    ...
    resolve_under_root(Path::new("/__odm_ws__"), path).map_err(|e| match e {
        PathResolveError::Absolute { path } => HiveError::hive(format!(
            "{kind} '{name}' path must be relative, got '{path}'"
        )),
        PathResolveError::Escape { path } => HiveError::hive(format!(
            "{kind} '{name}' path must not escape Hive root, got '{path}'"
        )),
    })?;
    Ok(())
}

fn validate_action_task_dir(action: &str, task_i: usize, dir: &str) -> Result<(), HiveError> {
    ...
    resolve_under_root(Path::new("/__odm_ws__"), dir).map_err(|e| match e {
        ...
        PathResolveError::Escape { path } => HiveError::hive(format!(
            "action '{action}' task {task_i} dir must not escape Hive root, got '{path}'"
        )),
    })?;
    Ok(())
}
...
fn validate_checkout(config: &HiveConfig) -> Result<(), HiveError> {
...
fn validate_progen_groups(config: &HiveConfig) -> Result<(), HiveError> {
...
fn resolve_bundle_path(
    ...
    resolve_under_root(root, rel).map_err(|e| match e {
        ...
        PathResolveError::Escape { path } => HiveError::hive(format!(
            "{kind} bundle '{bundle_name}' path must not escape Hive root, got '{path}'"
        )),
    })
}
...
fn load_action_bundles(
...
) -> Result<BTreeMap<String, ActionDef>, HiveError> {
    ...
    for (bundle_name, rel) in bundles {
        ...
        let text = fs::read_to_string(&path)
            .map_err(|e| HiveError::hive(format!("failed to read action bundle {rel}: {e}")))?;
        let map: BTreeMap<String, ActionDef> = if text.trim().is_empty() {
            BTreeMap::new()
        } else {
            serde_yaml::from_str(&text)
                .map_err(|e| HiveError::hive(format!("invalid action bundle {rel}: {e}")))?
        };
        for (name, def) in map {
...
fn load_generator_bundles(
    root: &Path,
    bundles: &BTreeMap<String, String>,
) -> Result<BTreeMap<String, GeneratorDef>, HiveError> {
    let mut merged = BTreeMap::new();
    for (bundle_name, rel) in bundles {
        ...
        let text = fs::read_to_string(&path).map_err(|e| {
            HiveError::hive(format!("failed to read generator bundle {rel}: {e}"))
        })?;
        let map: BTreeMap<String, GeneratorDef> = if text.trim().is_empty() {
            BTreeMap::new()
        } else {
            serde_yaml::from_str(&text)
                .map_err(|e| HiveError::hive(format!("invalid generator bundle {rel}: {e}")))?
        };
        for (name, def) in map {
            if merged.contains_key(&name) {
                return Err(HiveError::hive(format!(
...
/// Serialize and atomically write Hive config.
pub fn save_config(root: &Path, config: &HiveConfig) -> Result<(), HiveError> {
    let path = config_path(root);
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|e| {
            HiveError::operation(format!("failed to create {}: {e}", parent.display()))
        })?;
    }
    let yaml = serde_yaml::to_string(config)
        .map_err(|e| HiveError::operation(format!("failed to serialize config: {e}")))?;
