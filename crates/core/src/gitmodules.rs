...
pub fn desired_gitmodules(config: &HiveConfig) -> Option<String> {
...
pub fn gitmodules_has_drift(root: &Path, config: &HiveConfig) -> bool {
...
pub fn rewrite_gitmodules(root: &Path, config: &HiveConfig) -> Result<(), HiveError> {
    let path = gitmodules_path(root);
    match desired_gitmodules(config) {
        None => {
            if path.exists() {
...
mod tests {
    ...
    fn materialize_gitlink_rewrite_gitmodules_removes_when_none() {
        ...
        rewrite_gitmodules(root, &HiveConfig::default()).unwrap();
