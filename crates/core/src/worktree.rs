...
pub fn validate_slot_name(name: &str) -> Result<String, HiveError> {
    use crate::paths::parse_path_token;
    match parse_path_token(name) {
        Ok(t) => Ok(t.to_string()),
        Err("empty") => Err(HiveError::usage("worktree slot name must not be empty")),
        Err("dot") => Err(HiveError::usage(format!(
            "invalid worktree slot name '{}'",
            name.trim()
        ))),
        Err("separator") => Err(HiveError::usage(format!(
            "invalid worktree slot name '{}': path separators not allowed",
            name.trim()
        ))),
        Err(_) => Err(HiveError::usage(format!(
...
pub fn worktree_list<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    project: &str,
) -> Result<WorktreeListOutcome, HiveError> {
...
pub fn worktree_registered_names<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    project: &str,
) -> Result<BTreeSet<String>, HiveError> {
    let slots = list_registered_slots(git, ws, project)?;
    Ok(slots.into_iter().map(|s| s.name).collect())
}

fn list_registered_slots<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    project: &str,
) -> Result<Vec<WorktreeSlotInfo>, HiveError> {
...
pub fn worktree_add<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    project: &str,
    slot: &str,
    branch: Option<&str>,
) -> Result<WorktreeSlotOutcome, HiveError> {
...
pub fn worktree_rm<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    project: &str,
    slot: &str,
...
pub fn worktree_prune<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    project: &str,
    force: bool,
) -> Result<WorktreePruneOutcome, HiveError> {
...
pub fn worktree_prune_all<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    force: bool,
) -> Result<WorktreePruneAllOutcome, HiveError> {
    let mut pruned = Vec::new();
    let mut skipped_nonempty = Vec::new();
    ...
    for project in ws.config.projects.keys() {
        match worktree_prune(git, ws, project, force) {
            ...
            Err(HiveError::NotFound(_)) | Err(HiveError::Operation(_)) => continue,
            Err(e) => return Err(e),
        }
    }
...
pub fn worktree_orphans<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    project: &str,
) -> Result<Vec<WorktreeOrphanInfo>, HiveError> {
    let registered = worktree_registered_names(git, ws, project)?;
...
fn orphan_slot_names(ws: &Hive, project: &str, registered: &BTreeSet<String>) -> Vec<String> {
    let project_wt = ws.root.join("worktrees").join(project);
...
fn resolve_primary_git<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    project: &str,
) -> Result<PathBuf, HiveError> {
    let entry = ws.config.projects.get(project).ok_or_else(|| {
        HiveError::usage(format!("unknown project '{project}'"))
    })?;
