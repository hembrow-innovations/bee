//! Hive layout path policy — single owner for checkout, worktree, and index paths.
...
pub enum PathResolveError {
    ...
    #[error("path escapes Hive root: '{path}'")]
    Escape { path: String },
}
...
impl From<PathResolveError> for HiveError {
    fn from(err: PathResolveError) -> Self {
        HiveError::hive(err.to_string())
    }
}
...
/// Resolve config-relative `rel` under Hive `root`.
...
pub fn abs_checkout(root: &Path, rel: &str) -> Result<PathBuf, HiveError> {
...
pub fn path_buf_to_rel(path: &Path) -> Result<String, HiveError> {
...
mod tests {
    ...
    fn path_resolve_error_into_workspace_odm() {
        let abs: HiveError = PathResolveError::Absolute {
            path: "/x".into(),
        }
        .into();
        assert_eq!(abs.code(), "hive");
        assert!(abs.message().contains("relative"));
        let esc: HiveError = PathResolveError::Escape {
            path: "../y".into(),
        }
        .into();
        assert_eq!(esc.code(), "hive");
