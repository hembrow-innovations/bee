//! Hive membership — add/rm Project or Progen with kind-specific hooks.
...
use crate::config::{
    require_entity_name, save_config, CheckoutMode, ProjectEntry, ProgenEntry, HiveConfig,
};
...
/// Does not scaffold Progen vaults — that composition lives in `hive-store`.
pub fn membership_add<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    ...
    require_entity_name(label, name).map_err(|e| match e {
        HiveError::Hive(m) => HiveError::usage(m),
        other => other,
    })?;
    ...
    if already {
        return Err(HiveError::usage(format!("{label} '{name}' already exists")));
    }
    ...
    if entry.path().trim().is_empty() {
        return Err(HiveError::usage(format!("{label} path must not be empty")));
    }
    resolve_under_root(root, entry.path()).map_err(|e| match e {
        PathResolveError::Absolute { path } => {
            HiveError::usage(format!("{label} path must be relative, got '{path}'"))
        }
        PathResolveError::Escape { path } => HiveError::usage(format!(
            "{label} path must not escape Hive root, got '{path}'"
        )),
    })?;
...
pub fn membership_rm<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    config: &mut HiveConfig,
    kind: MembershipKind,
    ...
    match kind {
        ...
        MembershipKind::Progen => {
            let entry = config.progens.remove(name).ok_or_else(|| {
                HiveError::usage(format!("unknown progen '{name}'"))
            })?;
            if delete {
                maybe_delete_checkout(git, root, name, &entry.path, force, || {
...
fn remove_path(path: &Path) -> Result<(), HiveError> {
    if path.is_dir() {
        fs::remove_dir_all(path).map_err(|e| {
            HiveError::operation(format!("failed to delete {}: {e}", path.display()))
        })?;
    } else {
...
pub fn project_add<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    config: &mut HiveConfig,
    name: &str,
...
pub fn project_rm<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    config: &mut HiveConfig,
    name: &str,
...
/// Vault scaffold is composed by `hive-store`, not here.
pub fn progen_add<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    config: &mut HiveConfig,
    name: &str,
...
pub fn progen_rm<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    config: &mut HiveConfig,
    name: &str,
