...
pub fn is_dir_empty(path: &Path) -> Result<bool, HiveError> {
    let mut entries = fs::read_dir(path).map_err(|e| {
        HiveError::operation(format!("failed to read {}: {e}", path.display()))
    })?;
    Ok(entries.next().is_none())
}

/// Create `path` and parents as directories.
pub fn ensure_dir(path: &Path) -> Result<(), HiveError> {
    fs::create_dir_all(path).map_err(|e| {
        HiveError::operation(format!("failed to create {}: {e}", path.display()))
    })
}

/// Create parent directories of `path` when a parent exists.
pub fn ensure_parent(path: &Path) -> Result<(), HiveError> {
...
pub fn remove_path(path: &Path) -> Result<(), HiveError> {
    let meta = fs::symlink_metadata(path).map_err(|e| {
        HiveError::operation(format!("failed to stat {}: {e}", path.display()))
    })?;
    if meta.file_type().is_symlink() || meta.is_file() {
        fs::remove_file(path).map_err(|e| {
            HiveError::operation(format!("failed to remove {}: {e}", path.display()))
        })
    } else if meta.is_dir() {
...
pub fn remove_type_conflict(to: &Path, want_dir: bool) -> Result<(), HiveError> {
...
pub fn count_tree(src: &Path) -> Result<u32, HiveError> {
    let mut count = 0u32;
    let entries = fs::read_dir(src).map_err(|e| {
        HiveError::operation(format!("failed to read {}: {e}", src.display()))
    })?;
    for entry in entries {
        let entry = entry.map_err(|e| {
            HiveError::operation(format!("failed to read entry in {}: {e}", src.display()))
        })?;
        let from = entry.path();
        let ft = entry.file_type().map_err(|e| {
            HiveError::operation(format!("failed to stat {}: {e}", from.display()))
        })?;
        if ft.is_dir() {
...
pub fn copy_symlink(from: &Path, to: &Path) -> Result<(), HiveError> {
    let target = fs::read_link(from).map_err(|e| {
        HiveError::operation(format!("failed to read symlink {}: {e}", from.display()))
    })?;
...
pub fn create_symlink(target: &Path, link: &Path) -> Result<(), HiveError> {
