...
/// Build Hive status snapshot. Does not fetch.
pub fn build_status<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
) -> Result<StatusSnapshot, HiveError> {
    let pin = load_pin(&ws.root)?;
    let obs = crate::observation::observe_hive(git, &ws.root, &ws.config, pin.as_ref())?;
...
pub fn status_from_observation(obs: &crate::observation::HiveObservation) -> StatusSnapshot {
    StatusSnapshot {
        root: obs.root.clone(),
