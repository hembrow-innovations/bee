//! Local Generator materialize — copy a template directory under the Hive.

use std::fs;
use std::path::PathBuf;

use crate::config::{GeneratorDef, Hive};
...
pub struct GenerateOutcome {
    ...
    pub copied: u32,
    /// Absolute destination path under the Hive root.
    pub dest: PathBuf,
}

/// Resolve a Generator by name from the loaded Hive.
pub fn generator<'a>(ws: &'a Hive, name: &str) -> Result<&'a GeneratorDef, HiveError> {
    ws.generators
        .get(name)
        .ok_or_else(|| HiveError::usage(format!("unknown generator '{name}'")))
}

/// Materialize a local `template` directory into `dest_rel` under the Hive root.
///
...
pub fn generate_local(
    ...
    force: bool,
    dry_run: bool,
) -> Result<GenerateOutcome, HiveError> {
    ...
    if dest.exists() {
        let meta = fs::symlink_metadata(&dest).map_err(|e| {
            HiveError::operation(format!("failed to stat {}: {e}", dest.display()))
        })?;
        if !meta.is_dir() {
            return Err(HiveError::operation(format!(
        ...
        fs::create_dir(&dest).map_err(|e| {
            HiveError::operation(format!("failed to create {}: {e}", dest.display()))
        })?;
    } else {
