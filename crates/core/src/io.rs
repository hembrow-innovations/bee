...
pub fn atomic_write(path: &Path, contents: &str) -> Result<(), HiveError> {
...

    {
        let mut f = fs::File::create(&tmp).map_err(|e| {
            HiveError::operation(format!("failed to create temp {}: {e}", tmp.display()))
        })?;
        f.write_all(contents.as_bytes()).map_err(|e| {
            HiveError::operation(format!("failed to write temp {}: {e}", tmp.display()))
        })?;
        f.sync_all().ok();
    }
