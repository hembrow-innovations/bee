...
/// Desired inner lines for the Hive-root managed block (sorted, unique).
pub fn desired_workspace_lines(config: &HiveConfig) -> Vec<String> {
...
/// Seed/update Hive-root `.gitignore` managed block only.
...
/// Rewrite Hive-root and ancestor-checkout managed gitignore blocks.
///
/// No-op when `manage_gitignore` is false. Does not require Hive root to be a git
...
/// True when on-disk managed block at Hive root differs from desired (or missing).
...
fn on_disk_differs(gitignore_path: &Path, desired_inner: &[String]) -> Result<bool, HiveError> {
...
fn managed_paths(config: &HiveConfig) -> Vec<String> {
...
pub fn write_managed_block(gitignore_path: &Path, inner_lines: &[String]) -> Result<(), HiveError> {
