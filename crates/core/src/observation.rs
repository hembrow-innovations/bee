//! Hive observation — sample each declared entity once; derive `PinState`.
...
/// Full Hive observation snapshot (projects + progens).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct HiveObservation {
    pub root: String,
...
pub fn observe_hive<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    config: &HiveConfig,
    pin: Option<&PinFile>,
) -> Result<HiveObservation, HiveError> {
...
pub fn observe_entity<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    name: &str,
    rel_path: &str,
    ...
    pin: Option<&PinFile>,
) -> Result<EntityObservation, HiveError> {
    let pin_present = pin.is_some();
    ...
    let (abs_path, resolve_error, on_disk, is_git, head, origin, dirty) =
        match resolve_under_root(root, rel_path) {
            Ok(abs) => {
                ...
                // Own checkout only — not nested inside an ancestor Hive/monorepo git.
...
mod tests {
    ...
    fn pin_source_gitlink_observe_skips_missing_pin_file() {
        ...
        let git = Git::new();
        let obs = observe_hive(&git, dir.path(), &cfg, None).unwrap();
    ...
    fn observe_attaches_pin_state_via_classifier() {
        ...
        let obs = observe_hive(&git, dir.path(), &cfg, Some(&pin)).unwrap();
    ...
    fn path_only_under_workspace_git_is_not_is_git() {
        ...
        // Dirty the Hive git root — must not leak onto path-only progen.
