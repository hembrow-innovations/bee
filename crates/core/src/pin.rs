    ...
    pub fn validate(&self) -> Result<(), HiveError> {
        ...
        for (name, entry) in &self.pins {
            if name.trim().is_empty() {
                return Err(HiveError::hive("pin name must not be empty"));
            }
...
pub fn parse_pin_yaml(text: &str) -> Result<PinFile, HiveError> {
    let pin: PinFile = serde_yaml::from_str(text.trim()).map_err(|e| {
        HiveError::hive(format!("invalid pin YAML: {e}"))
    })?;
    pin.validate()?;
    Ok(pin)
}

pub fn load_pin(root: &Path) -> Result<Option<PinFile>, HiveError> {
...
pub fn save_pin(root: &Path, pin: &PinFile) -> Result<(), HiveError> {
    pin.validate()?;
    ...
    let yaml = serde_yaml::to_string(pin)
        .map_err(|e| HiveError::operation(format!("failed to serialize pin: {e}")))?;
