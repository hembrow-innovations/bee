...
pub fn all_managed(config: &HiveConfig) -> Vec<ManagedEntity> {
    let mut out = Vec::new();
...
pub fn resolve_managed(
    config: &HiveConfig,
    names: &[String],
) -> Result<Vec<ManagedEntity>, HiveError> {
    if names.is_empty() {
    ...
    for name in names {
        if let Some(e) = config.projects.get(name) {
            let url = e.url.as_ref().ok_or_else(|| {
                HiveError::usage(format!("project '{name}' is path-only (not managed)"))
            })?;
            out.push(ManagedEntity {
                name: name.clone(),
                path: e.path.clone(),
        ...
        if let Some(e) = config.progens.get(name) {
            let url = e.url.as_ref().ok_or_else(|| {
                HiveError::usage(format!("progen '{name}' is path-only (not managed)"))
            })?;
            out.push(ManagedEntity {
                name: name.clone(),
                path: e.path.clone(),
        ...
        return Err(HiveError::usage(format!("unknown entity '{name}'")));
    }
    Ok(out)
}
...
pub fn materialize<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    entity: &ManagedEntity,
) -> Result<MaterializeOutcome, HiveError> {
    ...
    let origin = match git.origin_url(&path) {
        Ok(u) => u,
        Err(hive_git::GitError::OriginMissing { .. }) => {
            return Err(HiveError::operation(format!(
...
fn materialize_gitlink<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    entity: &ManagedEntity,
) -> Result<MaterializeOutcome, HiveError> {
...
fn is_empty_dir(path: &Path) -> Result<bool, HiveError> {
    let mut rd = fs::read_dir(path)
        .map_err(|e| HiveError::operation(format!("failed to read {}: {e}", path.display())))?;
    Ok(rd.next().is_none())
}
...
pub fn sync_managed<R: hive_git::CommandRunner>(
    git: &Git<R>,
    root: &Path,
    config: &HiveConfig,
    names: &[String],
) -> Result<Vec<SyncResult>, HiveError> {
...
mod tests {
    ...
    fn materialize_origin_mismatch() {
        ...
        assert!(matches!(err, HiveError::Operation(_)));
    }
    ...
    fn materialize_gitlink_missing_path_missing_record_adds() {
        ...
        assert_eq!(
            ...
            hive_git::GitlinkRecord::Missing
        );
        assert_eq!(
            materialize(&g, &root, &entity).unwrap(),
            MaterializeOutcome::Cloned
        ...
        assert!(matches!(
            g.gitlink_record(&root, Path::new("vendor/nested")).unwrap(),
            hive_git::GitlinkRecord::Recorded { .. }
        ));
    ...
    fn materialize_gitlink_missing_path_recorded_inits() {
        ...
        let sha = match g.gitlink_record(&root, Path::new("vendor/nested")).unwrap() {
            hive_git::GitlinkRecord::Recorded { sha } => sha,
        ...
        assert_eq!(
            g.gitlink_record(&root, Path::new("vendor/nested")).unwrap(),
            hive_git::GitlinkRecord::Recorded { sha }
        );
        assert_eq!(
            materialize(&g, &root, &entity).unwrap(),
            MaterializeOutcome::Cloned
