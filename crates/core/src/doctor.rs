...
use hive_git::{Git, GitlinkRecord};
use serde::Serialize;

use crate::checkout::all_managed;
use crate::config::{odm_dir, pin_path, CheckoutMode, Hive};
...
use crate::observation::{observe_hive, EntityObservation};
...
pub fn run_doctor<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    fix: bool,
) -> Result<DoctorReport, HiveError> {
    if fix {
        apply_fixes(git, ws)?;
    }
...
fn apply_fixes<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
) -> Result<(), HiveError> {
...
fn ensure_odm_layout(root: &Path) -> Result<(), HiveError> {
    let odm = odm_dir(root);
    for name in ["cache", "log", "progen"] {
        let p = odm.join(name);
        fs::create_dir_all(&p).map_err(|e| {
            HiveError::operation(format!("failed to create {}: {e}", p.display()))
        })?;
    }
    Ok(())
}

fn collect_checks<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
) -> Result<Vec<DoctorCheck>, HiveError> {
...
fn push_entity_path_checks(
    ...
    checks.push(DoctorCheck {
        id: format!("path_declared:{id_base}"),
        status: CheckStatus::Pass,
        message: format!("{kind} '{name}' path under Hive root"),
        fixable: false,
    });
...
fn gitlink_checks<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
) -> Result<Vec<DoctorCheck>, HiveError> {
    let mut checks = Vec::new();
...
fn gitignore_drift_check<R: hive_git::CommandRunner>(
    ws: &Hive,
    git: &Git<R>,
) -> Result<DoctorCheck, HiveError> {
    ...
    if !is_git {
        return Ok(DoctorCheck {
            id: "gitignore_drift".into(),
            status: CheckStatus::Pass,
            message: "Hive is not a git repo".into(),
            fixable: false,
        });
    }
...
fn pin_checks(ws: &Hive) -> Result<Vec<DoctorCheck>, HiveError> {
    let mut checks = Vec::new();
    ...
    let text = fs::read_to_string(&path).map_err(|e| {
        HiveError::hive(format!("failed to read {}: {e}", path.display()))
    })?;
...
mod tests {
    ...
    use crate::config::{ProjectEntry, HiveConfig};
    use crate::init::{init_hive, InitOptions};
    use crate::pin::{save_pin, PinEntry, PinFile};

    fn load_ws(root: &Path) -> Hive {
        crate::config::load_hive(root).unwrap()
    }
