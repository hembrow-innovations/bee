//! `hive-actions` — resolve cwd + shell-out Action dispatch.

use std::path::{Path, PathBuf};
use std::process::Command;

use hive_core::{
    abs_checkout, resolve_under_root, worktree_slot_path, ActionTask, HiveError, PathResolveError,
    Hive,
};
...
impl<'a> CwdTarget<'a> {
    ...
    pub fn from_flags(project: Option<&'a str>, wt: Option<&'a str>) -> Result<Self, HiveError> {
        match (project, wt) {
            (None, Some(_)) => Err(HiveError::usage("--wt requires --project")),
...
pub fn list_actions(ws: &Hive) -> Vec<(&str, &hive_core::ActionDef)> {
    ws.actions
        .iter()
        .map(|(n, d)| (n.as_str(), d))
        .collect()
    ...
    if let Some(dir) = task_dir {
        let cwd = resolve_under_root(ws.root.as_path(), dir).map_err(|e| match e {
            PathResolveError::Absolute { path } => {
                HiveError::usage(format!("action task dir must be relative, got '{path}'"))
            }
            PathResolveError::Escape { path } => HiveError::usage(format!(
                "action task dir must not escape Hive root, got '{path}'"
            )),
        })?;
        if !cwd.is_dir() {
    ...
    match target {
        ...
        CwdTarget::Worktree { project, slot } => {
            if !ws.config.projects.contains_key(project) {
                return Err(HiveError::usage(format!("unknown project '{project}'")));
            }
...
pub fn run_action(
    ...
    action_name: &str,
    opts: RunOptions<'_>,
) -> Result<RunResult, HiveError> {
    let def = ws.actions.get(action_name).ok_or_else(|| {
        HiveError::usage(format!("unknown action '{action_name}'"))
    })?;
    ...
    match stdio {
        StdioMode::Inherit => {
            let status = cmd.status().map_err(|e| {
                HiveError::operation(format!("failed to spawn shell for action: {e}"))
            })?;
            Ok(TaskResult {
...
mod tests {
    ...
    use hive_core::{ActionDef, ActionTask, ProjectEntry, Hive, HiveConfig};
    use tempfile::tempdir;

    fn ws_with_actions(root: PathBuf, actions: BTreeMap<String, ActionDef>) -> Hive {
    ...
    fn resolve_cwd_missing_project_path_is_not_found() {
        ...
        assert!(matches!(err, HiveError::NotFound(_)), "{err:?}");
    ...
    fn resolve_cwd_unknown_project_is_usage() {
        ...
        assert!(matches!(err, HiveError::Usage(_)), "{err:?}");
