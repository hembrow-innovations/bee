...
impl ProgenStore {
    ...
    pub fn open(ws: &Hive, sp: &ScopedProgen) -> Result<Self, HiveError> {
    ...
    pub fn find(&self, query: &str, limit: usize) -> Result<Vec<FindHit>, HiveError> {
    ...
    pub fn list(&self) -> Result<Vec<LsHit>, HiveError> {
    ...
    pub fn get(&self, id: &str) -> Result<GetResult, HiveError> {
        let n = get_indexed(&self.conn, id)?.ok_or_else(|| {
            HiveError::not_found(format!("no such note: {id} in progen '{}'", self.name))
        })?;
        Ok(self.to_get(&n))
    }

    /// In-store wikilink neighborhood (no cross-store walk).
    pub fn context(&self, id: &str) -> Result<ContextHit, HiveError> {
        let n = get_indexed(&self.conn, id)?.ok_or_else(|| {
            HiveError::not_found(format!("no such note: {id} in progen '{}'", self.name))
        })?;
    ...
    pub fn tree(&self) -> Result<Vec<String>, HiveError> {
    ...
    pub fn backlinks(&self, id: &str) -> Result<Vec<LsHit>, HiveError> {
        Ok(self.context(id)?.incoming)
    }

    /// Rebuild index from vault; reconnects the handle.
    pub fn reindex(&mut self, ws: &Hive) -> Result<IndexStats, HiveError> {
        let sp = ScopedProgen {
        ...
        let _ = std::mem::replace(
            ...
            Connection::open_in_memory()
                .map_err(|e| HiveError::operation(format!("close index: {e}")))?,
        );
...
pub fn list_notes(ws: &Hive, progen: Option<&str>) -> Result<Vec<LsHit>, HiveError> {
...
pub fn get_note(ws: &Hive, id_arg: &str, progen: Option<&str>) -> Result<GetResult, HiveError> {
...
pub fn context_notes(
    ...
    id_arg: &str,
    progen: Option<&str>,
) -> Result<ContextHit, HiveError> {
    let (sp, id) = resolve_id(ws, id_arg, progen)?;
    ProgenStore::open(ws, &sp)?.context(&id)
}

/// Open single-root store for tree/backlinks/get-style CLI ops.
pub fn open_single(ws: &Hive, progen: Option<&str>) -> Result<ProgenStore, HiveError> {
    let sp = resolve_single_progen(ws, progen)?;
...
pub fn open_for_id(
    ...
    id_arg: &str,
    progen: Option<&str>,
) -> Result<(ProgenStore, String), HiveError> {
    let (sp, id) = resolve_id(ws, id_arg, progen)?;
    Ok((ProgenStore::open(ws, &sp)?, id))
}

/// Rebuild indexes for every Progen in `scope`.
pub fn reindex_scope(ws: &Hive, scope: &[ScopedProgen]) -> Result<Vec<IndexStats>, HiveError> {
...
pub fn reindex_for_cli(
    ws: &Hive,
    progen_flags: &[String],
) -> Result<Vec<IndexStats>, HiveError> {
...
) -> Result<Option<&str>, HiveError> {
    match flags {
        [] => Ok(None),
        [p] => Ok(Some(p.as_str())),
        _ => Err(HiveError::usage(usage_when_many)),
    }
}
...
fn resolve_id(
    ...
    id_arg: &str,
    progen: Option<&str>,
) -> Result<(ScopedProgen, String), HiveError> {
...
mod tests {
    ...
    fn ws_one(root: &std::path::Path, vault_rel: &str, name: &str) -> (Hive, ScopedProgen) {
    ...
    fn ws_two(root: &std::path::Path) -> Hive {
        for rel in ["va", "vb"] {
