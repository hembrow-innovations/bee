...
pub fn ensure_vault(path: &Path) -> Result<(), HiveError> {
    fs::create_dir_all(path).map_err(|e| {
        HiveError::operation(format!("failed to create vault {}: {e}", path.display()))
    })?;
    ...
    if !readme.exists() {
        ...
        .map_err(|e| HiveError::operation(format!("write README: {e}")))?;
    }

    let obsidian = path.join(".obsidian");
    if !obsidian.exists() {
        fs::create_dir_all(&obsidian).map_err(|e| {
            HiveError::operation(format!("create .obsidian: {e}"))
        })?;
        let app = obsidian.join("app.json");
        if !app.exists() {
            fs::write(&app, "{\n  \"legacyEditor\": false\n}\n").map_err(|e| {
                HiveError::operation(format!("write .obsidian/app.json: {e}"))
            })?;
        }
    }
    ...
    if !gitignore.exists() {
        fs::write(&gitignore, ".obsidian/workspace.json\n.obsidian/workspace-mobile.json\n.trash/\n")
            .map_err(|e| HiveError::operation(format!("write vault .gitignore: {e}")))?;
    }
...
pub fn walk_notes(vault: &Path) -> Result<Vec<NoteDoc>, HiveError> {
    if !vault.is_dir() {
...
fn walk_dir(root: &Path, dir: &Path, out: &mut Vec<NoteDoc>) -> Result<(), HiveError> {
    let entries = fs::read_dir(dir).map_err(|e| {
        HiveError::operation(format!("read {}: {e}", dir.display()))
    })?;
    for ent in entries {
        ...
        if path.is_dir() {
            ...
            let text = fs::read_to_string(&path).map_err(|e| {
                HiveError::operation(format!("read {}: {e}", path.display()))
            })?;
...
pub fn vault_info(sp: &ScopedProgen) -> Result<VaultInfo, HiveError> {
