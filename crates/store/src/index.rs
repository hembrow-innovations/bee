...
use hive_core::{progen_index_dir, HiveError, Hive};
...
fn vault_fingerprint(vault: &Path) -> Result<String, HiveError> {
    if !vault.is_dir() {
...
fn collect_fp(root: &Path, dir: &Path, out: &mut Vec<String>) -> Result<(), HiveError> {
    let entries = fs::read_dir(dir)
        .map_err(|e| HiveError::operation(format!("read {}: {e}", dir.display())))?;
    for ent in entries {
        let ent = ent.map_err(|e| HiveError::operation(e.to_string()))?;
        let path = ent.path();
        ...
        if path.is_dir() {
            ...
            let meta = fs::metadata(&path)
                .map_err(|e| HiveError::operation(format!("stat {}: {e}", path.display())))?;
            let nanos = meta
                .modified()
...
fn read_meta(conn: &Connection, key: &str) -> Result<Option<String>, HiveError> {
    let mut stmt = conn
        .prepare("SELECT value FROM meta WHERE key = ?1")
        .map_err(|e| HiveError::operation(e.to_string()))?;
    let mut rows = stmt
        .query_map(params![key], |row| row.get(0))
        .map_err(|e| HiveError::operation(e.to_string()))?;
    match rows.next() {
        Some(r) => Ok(Some(r.map_err(|e| HiveError::operation(e.to_string()))?)),
        None => Ok(None),
    }
}

fn index_stale(conn: &Connection, vault: &Path) -> Result<bool, HiveError> {
...
fn schema_current(conn: &Connection) -> Result<bool, HiveError> {
...
pub(crate) fn reindex_progen(ws: &Hive, sp: &ScopedProgen) -> Result<IndexStats, HiveError> {
    ...
    fs::create_dir_all(&dir)
        .map_err(|e| HiveError::operation(format!("create index dir {}: {e}", dir.display())))?;
    ...
    let conn =
        Connection::open(&db).map_err(|e| HiveError::operation(format!("open index: {e}")))?;
    conn.execute_batch(SCHEMA)
        .map_err(|e| HiveError::operation(format!("index schema: {e}")))?;
        ...
        .map_err(|e| HiveError::operation(e.to_string()))?;
    {
        ...
        for n in &notes {
            ...
            ins.execute(params![n.id.as_str(), n.rel_path, title, n.body])
                .map_err(|e| HiveError::operation(format!("index insert: {e}")))?;
            ins_fts
                .execute(params![n.id.as_str(), title, n.body])
                .map_err(|e| HiveError::operation(format!("fts insert: {e}")))?;
...
pub(crate) fn open_index(ws_root: &Path, progen_name: &str) -> Result<Connection, HiveError> {
...
pub(crate) fn ensure_index(ws: &Hive, sp: &ScopedProgen) -> Result<(), HiveError> {
    let db = index_db_path(&ws.root, &sp.name);
    if !db.exists() {
...
pub(crate) fn load_all_notes(conn: &Connection) -> Result<Vec<IndexedNote>, HiveError> {
    let mut stmt = conn
    ...
    let mut out = Vec::new();
    for r in rows {
        out.push(r.map_err(|e| HiveError::operation(e.to_string()))?);
    }
    Ok(out)
}
...
pub(crate) fn search_fts(
    ...
    query: &str,
    limit: usize,
) -> Result<Vec<IndexedNote>, HiveError> {
    ...
    let rows = match mapped {
        ...
        Err(e) => return Err(HiveError::operation(e.to_string())),
    };
...
pub(crate) fn get_indexed(conn: &Connection, id: &str) -> Result<Option<IndexedNote>, HiveError> {
    let mut stmt = conn
    ...
    if let Some(r) = rows.next() {
        return Ok(Some(r.map_err(|e| HiveError::operation(e.to_string()))?));
    }
...
pub(crate) fn outgoing_links(conn: &Connection, src_id: &str) -> Result<Vec<String>, HiveError> {
    let mut stmt = conn
...
pub(crate) fn resolve_link_target(
    conn: &Connection,
    target: &str,
) -> Result<Option<IndexedNote>, HiveError> {
...
pub(crate) fn backlinks_for(conn: &Connection, target: &str) -> Result<Vec<String>, HiveError> {
    let mut stmt = conn
        .prepare(
...
mod tests {
    ...
    fn ws_sp(root: &Path, vault_rel: &str, name: &str) -> (Hive, ScopedProgen) {
