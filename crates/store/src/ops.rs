...
pub fn doctor_progens(
    ws: &Hive,
    progen: Option<&str>,
) -> Result<Vec<ProgenDoctorCheck>, HiveError> {
    ...
    for sp in scope {
        ...
        let idx = hive_core::progen_index_dir(&ws.root, &sp.name).join("index.db");
...
mod tests {
    ...
    use hive_core::{progen_index_dir, ProgenEntry, HiveConfig};
