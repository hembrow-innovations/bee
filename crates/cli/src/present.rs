...
pub fn finish(out: &GlobalOut, value: &impl Present) -> Result<i32, HiveError> {
    if out.json {
...
pub fn print_json<T: Serialize>(value: &T) -> Result<(), HiveError> {
    let s = serde_json::to_string_pretty(value)
        .map_err(|e| HiveError::operation(format!("json encode failed: {e}")))?;
    println!("{s}");
    Ok(())
}

/// Print error (human stderr or JSON stdout) and return exit code.
pub fn print_error(out: &GlobalOut, err: &HiveError) -> i32 {
    let code = hive_core::exit_code(err);
    if out.json {
...
pub fn json_value<T: Serialize>(value: &T) -> Result<serde_json::Value, HiveError> {
