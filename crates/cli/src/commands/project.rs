...
use hive_core::{
    build_status, load_pin, observe_project_worktrees_soft, observe_hive, path_buf_to_rel,
    project_add, project_git, project_rm, CheckoutMode, EntityObservation, HiveError, PinState,
    ProjectEntry, StatusSnapshot, Hive, WorktreeOrphanInfo, WorktreeSlotInfo,
};
use hive_git::Git;
use serde::Serialize;
...
pub fn project_list_from(ws: &Hive, snap: &StatusSnapshot) -> ProjectListDto {
    let projects = ws
        .config
        .projects
        .iter()
...
pub fn list_projects<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
) -> Result<ProjectListDto, HiveError> {
...
pub fn project_info<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
    name: &str,
) -> Result<ProjectInfoDto, HiveError> {
    let entry = ws
        .config
        .projects
        .get(name)
        .ok_or_else(|| HiveError::usage(format!("unknown project '{name}'")))?;
    let pin = load_pin(&ws.root)?;
    let obs = observe_hive(git, &ws.root, &ws.config, pin.as_ref())?;
    let st = obs
        .projects
        .iter()
        .find(|p| p.name == name)
...
pub fn list_cmd(ctx: &Ctx) -> Result<ProjectListDto, HiveError> {
    list_projects(&ctx.git, &ctx.ws)
}

/// Handler: project info.
pub fn info_cmd(ctx: &Ctx, name: &str) -> Result<ProjectInfoDto, HiveError> {
...
mod tests {
    ...
    use hive_core::{
        EntityStatus, PinState, ProjectEntry, HiveConfig, WorktreeOrphanInfo, WorktreeSlotInfo,
    };

    fn ws_one(name: &str, path: &str) -> Hive {
