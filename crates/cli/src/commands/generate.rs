...
use hive_core::{generate_local, path_buf_to_rel, HiveError, Hive};
use serde::Serialize;
...
pub fn list_generators_dto(ws: &Hive) -> GeneratorListDto {
    let generators = ws
        .generators
        .iter()
...
mod tests {
    ...
    use hive_core::{GeneratorDef, Hive, HiveConfig};

    fn ws_with(generators: BTreeMap<String, GeneratorDef>) -> Hive {
        Hive {
