...
use hive_core::{
    build_status, path_buf_to_rel, CheckoutMode, HiveError, PinState, ProgenEntry, StatusSnapshot,
    Hive,
};
use hive_git::Git;
use hive_store::{
...
/// Pure projection: Hive config ⨝ status snapshot → list DTO.
pub fn progen_list_from(ws: &Hive, snap: &StatusSnapshot) -> ProgenListDto {
    let progens = ws
        .config
        .progens
        .iter()
...
pub fn list_progens<R: hive_git::CommandRunner>(
    git: &Git<R>,
    ws: &Hive,
) -> Result<ProgenListDto, HiveError> {
...
pub fn progen_info(ws: &Hive, name: &str) -> Result<ProgenInfoDto, HiveError> {
    let entry = ws
        .config
        .progens
        .get(name)
...
pub fn list_cmd(ctx: &Ctx) -> Result<ProgenListDto, HiveError> {
    list_progens(&ctx.git, &ctx.ws)
}

pub fn info_cmd(ctx: &Ctx, name: &str) -> Result<ProgenInfoDto, HiveError> {
...
pub fn add_cmd(
    ...
    no_clone: bool,
    gitlink: bool,
) -> Result<Ready<NamedMaterialize>, HiveError> {
...
pub fn rm_cmd(
    ...
    delete: bool,
    force: bool,
) -> Result<Ready<NamedOk>, HiveError> {
...
pub fn get_cmd(ctx: &Ctx, id: &str) -> Result<Ready<GetResult>, HiveError> {
...
pub fn body_cmd(ctx: &Ctx, id: &str) -> Result<Ready<BodyDto>, HiveError> {
    let progen = one_progen_flag(
        &ctx.progen,
...
pub fn tree_cmd(ctx: &Ctx) -> Result<Ready<TreeDto>, HiveError> {
...
pub fn backlinks_cmd(ctx: &Ctx, id: &str) -> Result<Ready<BacklinksDto>, HiveError> {
    let progen = one_progen_flag(
        &ctx.progen,
...
pub fn ls_cmd(ctx: &Ctx) -> Result<Ready<NotesDto>, HiveError> {
...
pub fn reindex_cmd(ctx: &Ctx) -> Result<Ready<ReindexDto>, HiveError> {
...
pub fn doctor_cmd(ctx: &Ctx) -> Result<Ready<ProgenDoctorDto>, HiveError> {
...
) -> Result<Ready<ContextHit>, HiveError> {
...
mod tests {
    ...
    use hive_core::{EntityStatus, PinState, ProgenEntry, HiveConfig};
