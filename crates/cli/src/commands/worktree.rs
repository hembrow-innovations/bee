...
use hive_core::{
    worktree_add, worktree_list, worktree_prune, worktree_prune_all, worktree_rm, HiveError,
...
fn slot_dto(s: &hive_core::WorktreeSlotInfo) -> WorktreeSlotDto {
    WorktreeSlotDto {
        name: s.name.clone(),
        path: s.path.clone(),
        dirty: s.dirty,
...
fn prune_slot_dto(s: &hive_core::WorktreePruneSlot) -> WorktreePruneSlotDto {
...
pub fn worktree_prune_all_dto(out: &WorktreePruneAllOutcome) -> WorktreePruneAllDto {
    fn map_slots(
        slots: &[hive_core::WorktreePruneAllSlot],
...
pub fn list_cmd(ctx: &Ctx, project: &str) -> Result<Ready<WorktreeListDto>, HiveError> {
...
pub fn add_cmd(
    ...
    slot: &str,
    branch: Option<&str>,
) -> Result<Ready<WorktreeSlotActionDto>, HiveError> {
...
pub fn prune_cmd(
    ...
    all: bool,
    force: bool,
) -> Result<Ready<serde_json::Value>, HiveError> {
    if all {
...
mod tests {
    ...
    fn prune_dto_json_shape() {
        let out = WorktreePruneOutcome {
            project: "alpha".into(),
            pruned: vec![hive_core::WorktreePruneSlot {
                name: "stale".into(),
                path: "worktrees/alpha/stale".into(),
            }],
            skipped_nonempty: vec![hive_core::WorktreePruneSlot {
                name: "full".into(),
    ...
    fn prune_all_dto_json_shape() {
        use hive_core::{WorktreePruneAllOutcome, WorktreePruneAllSlot};
