...
use hive_core::{
    format_doctor_human, format_status_human, init_hive, pin_apply, pin_record, pin_status,
    run_doctor, sync_managed, InitOptions, InitResult, HiveError, PinApplyResult, PinRecordResult,
...
pub fn init_cmd(
    ...
    no_git: bool,
    interactive: bool,
) -> Result<Ready<InitDto>, HiveError> {
    if interactive {
        return Err(HiveError::not_implemented("init --interactive"));
    }
...
pub fn sync_cmd(ctx: &Ctx, names: &[String]) -> Result<Ready<SyncDto>, HiveError> {
...
pub fn pin_record_cmd(
    ...
    names: &[String],
    force: bool,
) -> Result<Ready<PinRecordDto>, HiveError> {
...
pub fn pin_apply_cmd(
    ...
    names: &[String],
    force: bool,
) -> Result<Ready<PinApplyDto>, HiveError> {
...
pub fn pin_status_cmd(ctx: &Ctx, names: &[String]) -> Result<Ready<PinStatusDto>, HiveError> {
...
pub fn status_cmd(ctx: &Ctx) -> Result<Ready<StatusSnapshot>, HiveError> {
    let snap = hive_core::build_status(&ctx.git, &ctx.ws)?;
    let human = format_status_human(&snap);
    Ok(Ready::ok(snap, human))
}

pub fn doctor_cmd(ctx: &Ctx, fix: bool) -> Result<Ready<hive_core::DoctorReport>, HiveError> {
